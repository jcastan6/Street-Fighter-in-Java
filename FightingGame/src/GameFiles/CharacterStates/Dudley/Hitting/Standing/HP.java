...
public class HP extends HittingMove {
    ...
    public HP(){
        this.hurtbox = new Hurtbox(0,0,80,30,damage,2);
        this.animation = new HittingAnimation("Dudley", "HP",15,0,55);
        animation.start();
    }
    ...
    public void updateMove(long elapsedTime, int x, int y, boolean facingLeft){
        ...
        if(facingLeft) {
            hurtbox.updateHurtbox(x - 70, y+50);
        }
        else{
