...
import GameFiles.CharacterStates.Dudley.*;
import GameFiles.CharacterStates.Dudley.Hit.CrouchHit;
import GameFiles.CharacterStates.Dudley.Hit.Hit;
import GameFiles.CharacterStates.Dudley.Hit.HitMove;
import GameFiles.CharacterStates.Dudley.Hit.Knockback;
import GameFiles.CharacterStates.Dudley.Hitting.HittingMove;
import GameFiles.CharacterStates.Dudley.Hitting.Special.*;
import GameFiles.CharacterStates.Dudley.Hitting.Standing.*;
import GameFiles.CharacterStates.Dudley.Hitting.Starting;
import GameFiles.CharacterStates.Dudley.Hitting.Super.CorkscrewBlow;
import GameFiles.CharacterStates.Dudley.Hitting.Taunt;

import GameFiles.CharacterStates.Dudley.Hitting.Super.SuperMove;

import java.awt.*;
import java.util.ArrayList;
import java.util.HashMap;

public class Dudley extends GameCharacter{
    private int x;
    private int y;
    ...
    private HashMap<String, Move> moves = new HashMap<>();
    {
        ...
        moves.put("Uppercut", new UpperCut());
        moves.put("DuckingStraight", new DuckingStraight());
        moves.put("Win", new Win());
        moves.put("Lose", new Lose());
        moves.put("Counter", new Counter());
        moves.put("CrossCounter", new CrossCounter());
        moves.put("MGB", new MGB());
        moves.put("CorkscrewBlow", new CorkscrewBlow());
        moves.put("Starting", new Starting());
    }

    public Dudley(int x, int y, boolean facingleft) {
        String character = "Dudley";
        this.x = x;
        this.y = y;
        this.yfloor = y;
        this.health = 800;
        ...
        this.backWalkingAnimation = new StandbyAnimation(character, "WalkingB",22,-20,50);
        this.walkingAnimation = new StandbyAnimation(character, "WalkingF",20,-15,50);
        ...
        this.standingHitbox = new Hitbox(x+XCENTER,y+YCENTER, 125,225);
    ...
    public void hit(Rectangle collision, int side, int damage){
        if(currentMove instanceof Counter){
            ovverrideMove("CrossCounter");
        }

        else if(blocking && !hitStun){
            hitStun = true;
            health -= damage/5;
            activateMove("Block");
        }

        else if(!hitStun || (currentMove instanceof HitMove && ((HitMove) currentMove).allowsHit())){
            if(currentMove != moves.get("DuckingStraight") && currentMove != moves.get("CrossCounter"))
            {
                hitStun = true;
                health -= damage;
    ...
    public void drawImage(Graphics g) {
        ...
        if(!(currentMove instanceof Win || currentMove instanceof Lose)) {
            ...
            g.fillOval(x + 270 + (yfloor - y) / 5, yfloor + 360, 190 - (yfloor - y) / 5, 30);
         }
